...
module.exports = class NotifyWhenMuted {
    ...
    initSettingsValues() {
        config.settings[0].value = this.api.Data.load("notifyWhenDeafen") ?? config.settings[0].value;
        config.settings[1].value = this.api.Data.load("notifyServerMuted") ?? config.settings[1].value;
        config.settings[2].value = this.api.Data.load("audioUrl") ?? config.settings[2].value;
        config.settings[3].value = this.api.Data.load("audioVolume") ?? config.settings[3].value;
        config.settings[4].value = this.api.Data.load("delayBetweenNotifications") ?? config.settings[4].value;
    }
    ...
    async handleSpeaking(_, args, ret) {
        if (!config.settings[1].value && !MediaEngineStore.isSelfMute() && MediaEngineStore.isMute()
            || MediaEngineStore.isSelfDeaf() && !config.settings[0].value) return;
        if (ret && !this.isPlaying) {
            this.isPlaying = true;
            this.audio = new Audio(config.settings[2].value);
            this.audio.volume = config.settings[3].value / 100;
            this.audio.addEventListener('ended', async () => {
                await delay(config.settings[4].value);
                this.isPlaying = false;
            });
            this.audio.play();
        }
