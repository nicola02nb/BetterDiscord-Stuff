...
const { Webpack, Data, UI, Patcher, DOM, React, Utils } = BdApi;
const { Filters } = Webpack;
const [ DiscordModules, ApplicationStreamingStore, RunningGameStore, QuestsStore, ChannelStore, GuildChannelStore, {navigateToQuestHome}, QuestIcon, RestApi] = Webpack.getBulk(
...
const SettingsButtonModule = Webpack.getAllBySource("iconForeground:", "\"aria-label\"");
const CountBadgeModule = BdApi.Webpack.getBySource("\"renderBadgeCount\"");

const TopBarButton = Webpack.getBySource("badgePosition:");
const SettingsButton = Object.values(SettingsButtonModule).find(m => m.renderer );
const CountBadge = Object.values(CountBadgeModule).find(m => m.toString().indexOf("\"renderBadgeCount\"") > 0 );

const TopBarRender = Webpack.getBySource("leading:", ",windowKey:");
const QuestDiscoveryButton = Webpack.getBySource(".GLOBAL_DISCOVERY_TABS.map(");
const QuestHomeButton = Webpack.getBySource("QUESTS");
...
module.exports = class BasePlugin {
    ...
    QuestButton = ({ type }) => {
        ...
        const className = status.enrollable ? "quest-button-enrollable" : status.enrolled ? "quest-button-enrolled" : status.claimable ? "quest-button-claimable" : "";
        const tooltip = status.enrollable ? `${status.enrollable} Enrollable Quests` : status.enrolled ? `${status.enrolled} Enrolled Quests` : status.claimable ? `${status.claimable} Claimable Quests` : "Quests";
        ...
        if (type === "top-bar") {
            ...
            return React.createElement(SettingsButton.renderer, {
                tooltipText: tooltip,
                onContextMenu: undefined,
                onClick: navigateToQuestHome,
                disabled: navigateToQuestHome === undefined,
    ...
    QuestsCount = () => {
        ...
        return React.createElement(React.Fragment, null,
            status.enrollable > 0 && React.createElement(UI.Tooltip, { text: "Enrollable" },
            ({ onMouseEnter, onMouseLeave }) => React.createElement(CountBadge, {
                onMouseEnter: onMouseEnter,
                onMouseLeave: onMouseLeave,
                count: status.enrollable,
                color: "var(--status-danger)"
            })),
            status.enrolled > 0 && React.createElement(UI.Tooltip, { text: "Enrolled" },
            ({ onMouseEnter, onMouseLeave }) => React.createElement(CountBadge, {
                onMouseEnter: onMouseEnter,
                onMouseLeave: onMouseLeave,
            ...
            status.claimable > 0 && React.createElement(UI.Tooltip, { text: "Claimable" },
            ({ onMouseEnter, onMouseLeave }) => React.createElement(CountBadge, {
                onMouseEnter: onMouseEnter,
                onMouseLeave: onMouseLeave,
